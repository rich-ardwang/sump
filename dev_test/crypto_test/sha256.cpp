...
{
	char tmp[3];
	char *result = new char[num * 2 + 1];
	memset(result, 0, num * 2 + 1);
	int size = 1;
	for (int i = 0; i < num; i++) {
		size = size + 2;
...
std::string SHA256::generate(const std::string &in_str)
{
	//check input param
	std::string out_str;
	if (in_str.empty())
	...
	BYTE *in_byte = new BYTE[len + 1];
	if (nullptr == in_byte)
		return out_str;
	memcpy(in_byte, in_str.c_str(), len+1);

	//sha256 encode
	BYTE buf[SHA256_BLOCK_SIZE];
	sha256_init(&ctx);
	sha256_update(&ctx, in_byte, len);
	sha256_final(&ctx, buf);

	//dump sha256 message digest to string
	out_str = hexdump(buf, SHA256_BLOCK_SIZE);
