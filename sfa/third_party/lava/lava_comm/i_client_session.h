...
namespace lava
{
	namespace comm
	{
		struct i_client_session : public base::i_object
		{
			...
			virtual bool send_request(__lv_in i_message* req_msg, __lv_out i_message*& reply_msg) = 0; // use default timeout
			virtual bool send_request(__lv_in i_message* req_msg, __lv_in int32_t timeout_seconds, __lv_out i_message*& reply_msg) = 0;
			virtual uint32_t post_request(__lv_in i_message* req_msg, __lv_in int32_t cookie) = 0;
			virtual void subscribe_with_filter(__lv_in char* topics, __lv_in i_message* msg, __lv_in i_message_listener* listener) = 0;
			virtual void unsubscribe_with_filter(__lv_in char* topics, __lv_in i_message* msg, __lv_in i_message_listener* listener) = 0;
