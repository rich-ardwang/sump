...
namespace ss {
	...
	protected:
		...
		void close() ;
        void clear_expired() ;
        bool is_expired(const std::string& sfile);
	...
	public:
        ...
        int set_level_by_config(const char* cfg_path, const char* node_name) ;
		int set_stdout_level(int level) ;
		int set_tty_level(int level) ;
		int get_level() const ;
        int get_tty_level() const ;
...
#define DEFAULT_LOGGER get_logger()

#define LOGWRITE(level , format , ...) \
	do{									\
        ss::logger * log = DEFAULT_LOGGER ; \
		if(log == NULL || log->get_level() < level) \
			break ;								        \
