...
void CConfig::loadGlobalConfig()
{
    ...
    m_gConf.stCedaCliInfo.sCedaBindKeySpr = SS_BASE::t_string2string(buff);

    buff[0] = '\0';
    GetPrivateProfileString(_T("maincfg"), _T("cedaTopicSepr"), _T("."), buff, MAX_PATH, m_confPath.c_str());
    m_gConf.stCedaCliInfo.sCedaTopicSpr = SS_BASE::t_string2string(buff);

    buff[0] = '\0';
    GetPrivateProfileString(_T("maincfg"), _T("cedaLoginTimeout"), _T("5000"), buff, MAX_PATH, m_confPath.c_str());
    m_gConf.stCedaCliInfo.nCedaLoginTimeout = _ttoi(buff);

    buff[0] = '\0';
    GetPrivateProfileString(_T("maincfg"), _T("cedaStopTimeout"), _T("500"), buff, MAX_PATH, m_confPath.c_str());
    m_gConf.stCedaCliInfo.nCedaStopTimeout = _ttoi(buff);

    buff[0] = '\0';
    GetPrivateProfileString(_T("maincfg"), _T("cedaProxyLogLel"), _T("3"), buff, MAX_PATH, m_confPath.c_str());
    m_gConf.stCedaCliInfo.nCedaProxyLogLel = _ttoi(buff);

    buff[0] = '\0';
    GetPrivateProfileString(_T("maincfg"), _T("cedaProxyLogFile"), _T(".\\log\\CedaATSMQClient"), buff, MAX_PATH, m_confPath.c_str());
    m_gConf.stCedaCliInfo.sCedaProxyLogFile = SS_BASE::t_string2string(buff);

    buff[0] = '\0';
    GetPrivateProfileString(_T("maincfg"), _T("cedaSubsHandleCnt"), _T("1"), buff, MAX_PATH, m_confPath.c_str());
    m_gConf.stCedaCliInfo.nSubsAckHandleCnt = _ttoi(buff);
}

//SSMQServer log
ss::logger* GetSrvLogger() {
    static ss::logger sRet(".\\log\\SSMQ_CEDAMQ_Server");
