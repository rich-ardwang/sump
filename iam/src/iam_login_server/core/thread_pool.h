...
*Version     : 1.0
*Date        : 2019-10-16
*Desc        : 1.we can commit the variable params function or the lambda function to operate, 
*              and can get the return value.
*              2.it not support the class member function, but support the class static 
...
//define max thread num as a little small
#define MAX_THREAD_NUM			20
//default thread num supposing the CPU has 4 cores
...
public:
    inline threadpool(unsigned short size = DEFAULT_THREAD_NUM) : stoped_{ false } {
        if (size < 1)
            idl_thr_num_ = DEFAULT_THREAD_NUM;
        else
            idl_thr_num_ = (size > MAX_THREAD_NUM) ? MAX_THREAD_NUM : size;

        for (size = 0; size < idl_thr_num_; ++size) {
            pool_.emplace_back(
                [this] {
                    //worker thread function
                    //before release this thread pool object, it must complete all tasks.
...
public:
    ...
    auto commit(F &&f, Args &&... args)->std::future<decltype(f(args...))> {
        ...
        //typename std::result_of<F(Args...)>::type is return value of function f
