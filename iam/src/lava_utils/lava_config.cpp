...
bool lava_config::ConfigImpl::load()
{
	...
	try
	{
		...
		if (0 != result)
			log_error(lava_config_module, "get config file stat error! result:[%d]", result);
		else
	...
	{
		loaded_ = false;
		log_error(lava_config_module, "Config::ConfigImpl::init: %s open fail", settingName_.c_str());
		return false;
	}
	...
	loaded_ = true;
	log_info(lava_config_module, "Config::ConfigImpl::init: %s updated", settingName_.c_str());
	return true;
}
