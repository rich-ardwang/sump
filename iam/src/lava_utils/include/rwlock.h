...
private:
	...
	{
		if (flag)
			return atomic_fetch_and_bit_set(&n_lock_, n_write_bit_);
		else
			return atomic_fetch_and_bit_reset(&n_lock_, n_write_bit_);
	}

	int inline inc_read()
	{
		return atomic_fetch_and_add(&n_lock_, n_read_step_);
	}

	int inline dec_read()
	{
		return atomic_fetch_and_sub(&n_lock_, n_read_step_);
	}

	bool inline reading()
	{
		return 0 != (atomic_and_fetch(&n_lock_, 0xffffffff) & n_mask_read_);
	}
