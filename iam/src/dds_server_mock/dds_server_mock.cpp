...
#define dds_server_mock_module         "dds_server_mock"
    ...
    private:
        virtual void on_event(int32_t event_code, void* event_data) {
            if (event_code == event_login_success) {
                log_info(dds_server_mock_module, "receive login sucess.");
        ...
        virtual void on_exception(int32_t error_code, const char* what) {
            log_debug(dds_server_mock_module, "exception happened: erro_code = %d, desc = %s", error_code, what);
        }
            ...
            if (!m_parent->m_user_args.test_mode)
                ...
                if (msg) {
                    ...
                    if (body) {
                        ...
                        log_debug(dds_server_mock_module, "async:on_message:%d, msg_code:%d, ret_msg:%s, ret_body:%s", 
...
public:
    ...
    void start_work() {
        ...
        log_info(dds_server_mock_module, "notify_all start");
    ...
    void stop_work() {
        ...
        log_info(dds_server_mock_module, "notify_all stop");
...
private:
    void work() {
        log_info(dds_server_mock_module, "client work thread.");
        test_user_login_req();
    }
    ...
    void timer_sub_cb() {
        ...
        m_sub_replys = 0;
        log_info(dds_server_mock_module, "publish messages, average = %d.", sub_replys / m_interval);
    }
    ...
    void timer_request_cb() {
        ...
        m_replys = 0;
        m_requests = 0;
        log_info(dds_server_mock_module, "replys average = %d; requests average = %d.", replys / m_interval, requests / m_interval);
    }
    ...
    void test_user_login_req() {
        ...
        log_info(dds_server_mock_module, "test_user_login_req in %d.", m_is_start);
        ...
        while (true) {
            ...
            if (m_user_args.test_mode) {
                ...
                msg = create_message(tmp_tkinfo.u_name, tmp_tkinfo.token);
            }
            else {
    ...
    i_message* create_message(const char *name, const char *token) {
        std::string user_name(name);
        std::string user_token(token);
        i_message* msg = get_message(MSG_TYPE_DATA_REQ);
        msg->get_message_body()->add_int(4, 0);
        msg->get_message_body()->add_int(5, 2);
        std::string in_value;
        in_value += user_name;
        in_value += ",";
        in_value += user_token;
    ...
    i_message* create_message(const int sid) {
        i_message* msg = get_message(MSG_TYPE_DATA_REQ);
        msg->get_message_body()->add_int(4, 0);
        msg->get_message_body()->add_int(5, 2);
        std::string in_value = "id000";
        in_value += std::to_string(sid);
        in_value += ",";
        in_value += "336df8681794f308f244da4532ad07efe73ee39f3f4fd5c42177288914da70e0";
        msg->get_message_body()->add_string(6, in_value.c_str());
        return msg;
    }
...
void mock_dds_server(user_args_type& user_args) {
    log_info(dds_server_mock_module, "as lava client: io-thread = %d, worker-thread = %d", user_args.io_threads, user_args.work_threads);
